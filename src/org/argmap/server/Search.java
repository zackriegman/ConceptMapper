...
public class Search {
	...
	public Search(Objectify ofy, Set<String> tokenSet,
	...
	public PropsAndArgs getBatch(Objectify ofy, int limit) {
		...
		if (currentCombination != null && cursor != null) {
			currentQuery = buildQueryFromCurrentCombination(ofy);
			currentQuery.startCursor(cursor);
		}
		...
		while (true) {
			...
			if (currentQuery != null) {
				QueryResultIterator<Proposition> iterator = currentQuery
						.iterator();
				...
				while (iterator.hasNext()) {
					 ...
					 */
					if (filterIDs.add(proposition.id)) {
						/* add the result */
						...
						if (results.size() == limit) {
							...
							log.severe("returning " + results.size() + " results after reaching limit");
							return propsAndArgs;
						}
					}
				}
			 ...
			 */
				...
				log.severe("returning " + results.size() + " results after exchausting search combinations");
				return propsAndArgs;
			}
		}
	}
